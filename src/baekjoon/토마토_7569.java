...
public class 토마토_7569 {
    ...
    private static class Tomato {
        ...
        Tomato(int h, int x, int y, int day) {
            this.h = h;
            this.x = x;
            this.y = y;
            this.day = day;
    ...
    public static void main(String[] args) throws IOException {
        ...
        int unRipe = 0;
        List<Tomato> ripe = new ArrayList<>();
        ...
        for (int k = 0; k < H; k++) {
            for (int i = 0; i < N; i++) {
                ...
                for (int j = 0; j < M; j++) {
                    ...
                    if (tomatoes[k][i][j] == 0) unRipe++;
                    else if (tomatoes[k][i][j] == 1) {
                        ripe.add(new Tomato(k, i, j, 0));
                    }
                }
            }
        }
        ...
        if (unRipe == 0) {
            System.out.println(0); // 처음부터 모든 토마토가 익어 있는 상태
        }
        else {
            System.out.println(bfs(ripe));
        }
    }

    private static int bfs(List<Tomato> ripe) {
        int day = 0;
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < 6; i++) {
                ...
                if (tomatoes[nh][nx][ny] != 0) continue;

                visit[nh][nx][ny] = true;
                tomatoes[nh][nx][ny] = 1;
                queue.offer(new Tomato(nh, nx, ny, day + 1));
            }
        }

        // 만약 익지 않은 토마토가 있다면 -1 리턴
        if (!checkUnRipe()) {
            return -1;
        } else return day;
    }

    private static boolean checkUnRipe() {
        for (int k = 0; k < H; k++) {
            for (int i = 0; i < N; i++) {
                for (int j = 0; j < M; j++) {
                    if (tomatoes[k][i][j] == 0) return false;
                }
            }
        }
